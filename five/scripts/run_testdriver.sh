...
# Usage: ../bin/validate_five enroll_1N|finalize_1N|search_1N -c configDir -e enrollDir -o outputDir -h outputStem -i inputFile -t numForks
...
../common/scripts/count_threads.sh validate_five $outputDir/thread.log & pid=$!

# Enrollment
inputFile=input/enroll_1N.txt
bin/validate_five enroll_1N -c $configDir -o $outputDir -h $outputStem -i $inputFile -t $numForks
retEnrollment=$?
...
bin/validate_five enroll_1N -c $tempConfigDir -o $outputDir -h $outputStem -i $inputFile -t $numForks
retEnrollment=$?
if [[ $retEnrollment == 0 ]]; then
    echo "[SUCCESS]"
else
...
# Enrollment
inputFile=input/enroll_1N.txt
bin/validate_five $modality enroll_1N -c $configDir -o $outputDir -h $outputStem -i $inputFile -t $numForks
retEnrollment=$?
if [[ $retEnrollment == 0 ]]; then
    echo "[SUCCESS]"
    # Merge output files together
...
# Finalization
bin/validate_five finalize_1N -c $configDir -e $enrollDir -o $outputDir -h $outputStem
retFinalize=$?
...
bin/validate_five search_1N -c $configDir -e $enrollDir -o $outputDir -h $outputStem -i $inputFile -t $numForks
retSearch=$?
if [[ $retSearch == 0 ]]; then
    echo "[SUCCESS]"
    # Merge output files together
